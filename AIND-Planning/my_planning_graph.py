...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        :return:
            adds A nodes to the in self.a_ current levellevels[level]
        """
        a_nodes = []
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
           ...
           # we can add without fear of duplication, no if statement needed
            for node_s in node_a.effnodes:
                # an action node is added to be made reachable 
                s_nodes.add(node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # making sure that the action at node A1 has effect and at A2 is its negation
        for e in node_a1.action.effect_add:
            if e in node_a2.action.effect_rem:
                return True

        # making sure that the action at node A2 has effect and at A1 is its negation
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Make sure node_a1's don't conflict with node_a2'spreconditions
        # Make sure node_a2's don't conflict with node_a1'spreconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #making sure the  conditions aren't mutually exclusive
        for nodeA in node_a1.parents:
            for nodeB in node_a2.parents:
                if (nodeA.is_mutex(nodeB)):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        """
        #making sure the  conditions aren't mutually exclusive
        # referrred to this website : http://planning.cs.uiuc.edu/node66.html
    ...
    def h_levelsum(self) -> int:
        ...
        # for each goal in the problem, determine the level cost, then add them together
         ...
         # For each goal in the problem
        for goal in self.problem.goal:
            goal_found = False

            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
          # determine their level cost, then add them together
